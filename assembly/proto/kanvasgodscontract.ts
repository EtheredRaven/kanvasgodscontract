...
export namespace kanvasgodscontract {
  @unmanaged
  ...
  export class tokens_of_arguments {
    static encode(message: tokens_of_arguments, writer: Writer): void {
      if (message.owner.length != 0) {
        writer.uint32(10);
        writer.bytes(message.owner);
      }
    ...
    static decode(reader: Reader, length: i32): tokens_of_arguments {
      const end: usize = length < 0 ? reader.end : reader.ptr + length;
      const message = new tokens_of_arguments();
  ...
  export class tokens_of_result {
    static encode(message: tokens_of_result, writer: Writer): void {
      const unique_name_token_id = message.token_id;
      if (unique_name_token_id.length !== 0) {
        for (let i = 0; i < unique_name_token_id.length; ++i) {
          writer.uint32(10);
          writer.string(unique_name_token_id[i]);
        }
      }
    }

    static decode(reader: Reader, length: i32): tokens_of_result {
      const end: usize = length < 0 ? reader.end : reader.ptr + length;
      const message = new tokens_of_result();

      while (reader.ptr < end) {
        const tag = reader.uint32();
        switch (tag >>> 3) {
          case 1:
            message.token_id.push(reader.string());
            break;
    ...
    constructor(token_id: Array<string> = []) {
      this.token_id = token_id;
    }
  }
